...
| student_status/part_4_graduation_and_terminal_states | stateDiagram-v2 | 4 | 6 | Yes | Yes (S4.0?, S4.1, S4.2) | Yes | Yes | Needs Review |
...
- **Needs Review** — Contains Medium-certainty edges or a labeling judgment (e.g. `S4.0 [Terminal?]`, SAP escalation, direct-enrollment omission) that an analyst should sanity-check.
...
- [x] Terminal states labeled `[Terminal]` (or `[Terminal?]` for S4.0).
